...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        return (state[0]>=0 and state[0]<n and state[1]>=00 and state[1]<n and state not in walls)
    ...
    def successor(self, state):
        ...
        for i in range(1,4):
            ...
            if self.check_valid((newx,newy),self.walls, self.n):
                successors[f'Gore {i}']=(newx, newy)
        for i in range(1,4):
            newx, newy = x+i,y
            if self.check_valid((newx,newy),self.walls, self.n):
                successors[f'Desno {i}']=(newx, newy)
        return successors

    def h(self, node):
        x,y=node.state
        return (abs(self.goal[0]-x)+abs(self.goal[1]-y))/3
