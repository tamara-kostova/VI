...
if __name__ == '__main__':
    ...
    x = int(input())
    train_set = dataset[int(x*len(dataset)/100.0):]
    train_x = [row[:-1] for row in train_set]
    train_y = [1 if row[-1]>=5 else 0 for row in train_set]
    test_set = dataset[:int(x*len(dataset)/100.0)]
    ...
    col = importances.index(min(importances))

    train_x = [[row[i] for i in range(len(row)) if i!=col] for row in train_x]
    test_x = [[row[i] for i in range(len(row)) if i!=col] for row in test_x]
    classifier2 = MLPClassifier(hidden_layer_sizes=15,random_state=0)
    ...
    acc1, acc2 = 0, 0
    predictions1 = classifier2.predict(scaler1.transform(test_x))
    for i,prediction in enumerate(predictions1):
        if prediction == test_y[i]:
            acc1+=1
    print(f'Tocnost so StandardScaler: {acc1/len(test_set)}')
    classifier2.fit(scaler2.transform(train_x), train_y)
    predictions2 = classifier2.predict(scaler2.transform(test_x))
    for i, prediction in enumerate(predictions2):
        if prediction == test_y[i]:
            acc2 += 1
    print(f'Tocnost so MinMaxScaler: {acc2 / len(test_set)}')