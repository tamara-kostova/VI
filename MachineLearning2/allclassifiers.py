...
if __name__ == '__main__':
    ...
    train_set1 = class_0[:int(x1*len(class_0))]+class_1[:int(x1*len(class_1))]+class_2[:int(x1*len(class_2))]
    train_x1 = [row[:-1] for row in train_set1]
    train_y1 = [row[-1] for row in train_set1]
    nbclassifier = GaussianNB()
    nbclassifier.fit(train_x1,train_y1)

    train_set2 = class_0[int(x1*len(class_0)):int(x2 * len(class_0))] + class_1[int(x1*len(class_1)):int(x2 * len(class_1))] + class_2[int(x1*len(class_2)):int(x2 * len(class_2))]
    train_x2 = [row[:-1] for row in train_set2]
    train_y2 = [row[-1] for row in train_set2]
    tree = DecisionTreeClassifier(random_state=0)
    tree.fit(train_x2,train_y2)

    train_set3 = class_0[:int(x2 * len(class_0))] + class_1[:int(x2 * len(class_1))] + class_2[:int(x2 * len(class_2))]
    train_x3 = [row[:-1] for row in train_set3]
    train_y3 = [row[-1] for row in train_set3]
    forest = RandomForestClassifier(random_state=0)
    forest.fit(train_x3,train_y3)

    test_set = class_0[int(x2 * len(class_0)):] + class_1[int(x2 * len(class_1)):] + class_2[int(x2 * len(class_2)):]
    test_x = [row[:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]

    accuracy=0
    for i in range(len(test_set)):
        pred1 = nbclassifier.predict([test_x[i]])[0]
