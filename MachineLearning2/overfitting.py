...
if __name__ == '__main__':
    ...
    lr = float(input())
    col = int(input())
    zapis = [float(split) for split in input().split(" ")]

    scaler = MinMaxScaler(feature_range=(-1,1))
    train_set = dataset[:int(0.8*len(dataset))]
    train_x = [row[:-1] for row in dataset]
    train_y = [row[-1] for row in dataset]
    scaler.fit(train_x)

    classifier = MLPClassifier(neurons, learning_rate_init=lr, activation="relu", max_iter=20, random_state=0)
    ...
    if acc_train>1.15*acc_test:
        ...
        scaler2.fit(train_x)
        classifier2 = MLPClassifier(neurons, learning_rate_init=lr, activation="relu", max_iter=20, random_state=0)
        classifier2.fit(scaler2.transform(train_x),train_y)
        print("Se sluchuva overfitting")
        zapis.pop(col)
        print(classifier2.predict(scaler2.transform([zapis]))[0])
    else:
        print("Ne se sluchuva overfitting")
        print(classifier.predict(scaler.transform([zapis]))[0])
