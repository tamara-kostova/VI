            ...
            for j in range(self.n):
                ...
                if i!=j and state[i]!=():
                    if state[j]==() or state[i][-1]<=state[j][-1]:
                        new = list(state)
                        prvanew = list(state[i])
                        vtoranew = list(state[j])
                        prvanew.remove(state[i][-1])
                        vtoranew.append(state[i][-1])
                        new[i]=tuple(prvanew)
                        new[j]=tuple(vtoranew)
                        successors[f'MOVE TOP BLOCK FROM PILLAR {i+1} TO PILLAR {j+1}'] = tuple(new)
        return successors
...
if __name__ == "__main__":
    #pocetna = input()
    pocetnikuli = input().split(";")
    n = len(pocetnikuli)
    krajnikuli = input().split(";")
    initial = tuple([tuple(map(int, kula.split(','))) if kula != '' else () for kula in pocetnikuli])
    goal = tuple([tuple(map(int, kula.split(','))) if kula != '' else () for kula in krajnikuli])
    kuli = Hanoi(initial,goal,n)
    res = breadth_first_graph_search(kuli).solution()
    print(f'Number of action {len(res)}')
    print(res)