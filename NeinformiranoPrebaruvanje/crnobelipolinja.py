    ...
    def successor(self, state):
        ...
        for i in range(self.n):
            for j in range(self.n):
                new = [list(x) for x in state]
                new[i][j]=1 if new[i][j]==0 else 0
                if i-1>=0:
                    new[i-1][j]=1 if new[i-1][j]==0 else 0
                if j-1 >= 0:
                    new[i][j-1] = 1 if new[i][j-1] == 0 else 0
                if i+1 < n:
                    new[i+1][j] = 1 if new[i+1][j] == 0 else 0
                if j+1 < n:
                    new[i][j+1] = 1 if new[i][j+1] == 0 else 0
                initial = [tuple(x) for x in new]
                successors[f'x: {i}, y: {j}'] = tuple(initial)
        return successors
...
if __name__ == "__main__":
    ...
    for i in range(n):
        ...
        goalred = list()
        for j in range(n):
            red.append(polinja[k])
            goalred.append(1)
            k+=1
        init.append(tuple(red))
        goal.append(tuple(goalred))
    pole = Pole(tuple(init),tuple(goal),n)
    print(breadth_first_graph_search(pole).solution())