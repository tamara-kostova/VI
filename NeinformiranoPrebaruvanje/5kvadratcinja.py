...
class Squares(Problem):
    ...
    def successor(self, state):
        succ = {}
        #k1, k2, k3, k4, k5 = state[0], state[1], state[2], state[3], state[4]
        for i in range(5):
            x,y=state[i]  # x=k1_x, y=k1_y
            new = list(state)
            new[i]=(x,y+1)
            if self.check_valid(new):
                succ[f'Pomesti kvadratche {i+1} gore']=tuple(new)
            new[i] = (x, y - 1)
            if self.check_valid(new):
                succ[f'Pomesti kvadratche {i+1} dolu']=tuple(new)
            new[i] = (x-1, y)
            if self.check_valid(new):
                succ[f'Pomesti kvadratche {i+1} levo']=tuple(new)
            new[i] = (x + 1, y)
            if self.check_valid(new):
                succ[f'Pomesti kvadratche {i+1} desno']=tuple(new)
        return succ
